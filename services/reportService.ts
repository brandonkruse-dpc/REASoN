...
import { Student, YearGroup } from '../types';
...
export async function generateWeeklyPDF(students: Student[]) {
  ...
  // Helper for sorting and slicing
  ...
  const renderSection = (title: string, data: Student[]) => {
    ...
    data.forEach((s, i) => {
      ...
      // Color code risk score text
      if (s.riskScore > 70) doc.setTextColor(220, 38, 38); // red-600
      else if (s.riskScore > 40) doc.setTextColor(234, 88, 12); // orange-600
